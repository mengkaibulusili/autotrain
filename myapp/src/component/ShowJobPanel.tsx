...
import { Row, Col,Upload,Input,Button,message,Table,Select, Popover } from "antd";
...
export let ShowJobPanel = ()=>{
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text: any, record: any, index: number) => {
              ...
              return (
                <Popover key="detailInfoPopover" trigger="hover" placement="left" title={"任务详情"} content={viewContent()} >
                    <Button  key="detailInfoButton" type="default" icon={<QuestionCircleTwoTone  key="detailInfoTone" />} ></Button>
                </Popover> 
              )
            },
        },
        {
            ...
            key: "trainInfo",
            render: (text: any, record: any, index: number) => {
              return <Button key="trainInfoButton"  type="default"
    ...
    return (
        <Row gutter={10} justify="center">
            <Col>
                <Row gutter={10} justify="center"> 模糊匹配搜索部分数据  样例: 日期格式 20200906-12:44:32 </Row>
                <Row gutter={10} justify="center">
                    <Col>
                    <Input addonBefore={"模型名称"} value={modelName}  onChange={(event)=>{
                        let textValue = event.target.value;
                        setModelName(textValue);
                    }}  />
                    </Col>
                    <Col>
                    <Input addonBefore={"日期"} value={createTime}  onChange={(event)=>{
                        let textValue = event.target.value;
                        setCreateTime(textValue);
                    }}  />
                    </Col>
                    <Col>
                     <Select defaultValue="" style={{ width: 120 }} onChange={(value)=>{
                        setJonStatues(value);
                     }}>
                ...
                <Table rowKey={(record)=>_.get(record,"pk","x")} dataSource={dataS} columns={columns} />
            </Col>
        </Row>
    )
}